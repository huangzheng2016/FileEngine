...
### 不要整体归类的目录（需要递归处理子项）
- **名称含"待整理"、"Untreated"、"temp"、"下载"、"Downloads"、"杂项"、"懒得整理"**的目录 — 内容混杂
- **工具集合目录**：一个大目录下有多个独立工具/软件子目录 — 每个子目录是独立单元
- **备份根目录**：如 "Backup"、"Archive"、"Remote" — 只是容器，子目录才是实际内容
- **按时间段/阶段组织的父目录**：如 "Old"、"2023"、"高中"、"大学" — 只是分组容器，子目录才是独立单元。如果子目录的命名模式与外层同级目录一致（如都是 YYYYMMDD+事件），应归入同一个分类，不要因为中间有一层分组目录就分开处理
- **大量散文件混杂的目录**：如果一个目录下有几百个不同类型的文件（pdf、docx、zip、图片混在一起），不要整体归类，应逐个或按类型分批处理

### 散文件分类技巧
当遇到大量散落的文件时，按文件名和扩展名判断分类：
- **发票/收据**：文件名含"发票"、"dzfp"、"invoice"、"receipt" 或以票据编号命名的 PDF
- **数据库备份**：.sql、.sql.gz、含 "backup"/"dump" 的文件
- **源代码文件**：.go/.py/.js/.java 等，如果同目录有 go.mod/package.json 等，说明是散落的项目文件，应找到项目根目录整体归类
- **文档**：.pdf/.docx/.xlsx/.pptx，按内容主题分类（工作报告、学习资料、个人文档等）
- **压缩包**：按文件名判断内容归类，不要仅因为是压缩包就放到同一个分类
- **配置/密钥文件**：.json/.yaml/.conf/.pem/.key，通常属于某个项目或服务
